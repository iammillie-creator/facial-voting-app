...
def register():
    if request.method == 'POST':
        ...
        name = email = user_id = password = face_image_data = None

        # Check if request is JSON
        if request.content_type == 'application/json':
            print("DEBUG - Detected JSON request")
            try:
        ...
        # Otherwise handle form submission
            ...
            print("DEBUG - Detected form-data request")
            name = request.form.get('name')
            email = request.form.get('email')
            user_id = request.form.get('user_id')
            password = request.form.get('password')
            face_image_data = request.form.get('face_image_data')  # might be a hidden input with base64 image

        print(f"DEBUG - Email: {email}, User ID: {user_id}")
        print(f"DEBUG - Face image present: {bool(face_image_data)}")
        ...
        if existing_user:
            ...
            msg = 'Email or ID already registered'
            if request.content_type == 'application/json':
        ...
        # === Process Face Image ===
        if not face_image_data or not face_image_data.startswith('data:image'):
            print("DEBUG - Missing or invalid face image data")
            msg = 'Valid face image is required'
            if request.content_type == 'application/json':
                return jsonify({'success': False, 'message': msg}), 400
            flash(msg, 'danger')
            return redirect(url_for('register'))
        ...
        try:
            ...
            print(f"DEBUG - Saved face image to {face_path}")
